...
class plot_tool():
    ...
    def plot_learning_curve(self, std_area, color, interval):
        ...
        for round in range(self.n_round):
            # # metric_prev = 0
            # for sample in range(1, plot_size + 1):
            # acc_method[round] = accuracy_score(self.result_method[round, :sample * interval], self.result_true[round, : sample * interval])
            # print(len(self.result_method[round]), len(self.result_true[round]))
            # print(len(acc_method[round]))
