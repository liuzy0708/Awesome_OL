...
class ROALE_DI_strategy(object):
    def __init__(self, x_train, y_train, label_ratio=0.05, chunk_size=150, L=3, step=0.01, theta=0.005, D=10,
                 sigma_imbalance=0.01, clf=NaiveBayes()):
        ...
        if is_first == False:
            for i in range(self.L):
                if Hn[i] < self.I * labeling_ratio / self.L:
                    R_n_x = R_n_x + self.U_x[i][
                                    int(len(self.U_x[i]) - self.I * labeling_ratio / self.L + Hn[i]): len(self.U_x[i])]
                    R_n_y = R_n_y + self.U_y[i][
                                    int(len(self.U_y[i]) - self.I * labeling_ratio / self.L + Hn[i]): len(self.U_y[i])]
